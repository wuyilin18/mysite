...
Fluid.plugins = {
  ...
  imageCaption: function (selector) {
    ...
    jQuery(
      ...
      if ($figcaption.length !== 0) {
        ...
        if (imageTitle) {
          $target.after(
            `<figcaption aria-hidden="true" class="image-caption">${imageTitle}</figcaption>`
          );
        }
      }
    });
  ...
  codeWidget() {
    var enableLang =
      CONFIG.code_language.enable && CONFIG.code_language.default;
    ...
    function getBgClass(ele) {
      return Fluid.utils.getBackgroundLightness(ele) >= 0
        ? "code-widget-light"
        : "code-widget-dark";
    }
    ...
    jQuery(".markdown-body pre").each(function () {
      ...
      if (enableLang) {
        ...
        if (
          $pre[0].children.length > 0 &&
          $pre[0].children[0].classList.length >= 2 &&
          $pre.children().hasClass("hljs")
        ) {
          lang = $pre[0].children[0].classList[1];
        } else if ($pre[0].getAttribute("data-language")) {
          lang = $pre[0].getAttribute("data-language");
        } else if (
          $pre.parent().hasClass("sourceCode") &&
          $pre[0].children.length > 0 &&
          $pre[0].children[0].classList.length >= 2
        ) {
          lang = $pre[0].children[0].classList[1];
          ...
          $pre.parent().hasClass("markdown-body") &&
          $pre[0].classList.length === 0
        ) {
